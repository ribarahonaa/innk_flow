...
- **El stack quedó bajado: lo bajó Raúl al cerrar la sesión.** `make up` lo
  repone. **El volumen `innk_flow_postgres_data` sobrevive**, así que la base
  está intacta, incluido el desafío hecho a mano que no está en `db/seeds.rb`.
